...
module Awestruct
  module Extensions
    class Userguide
      ...
      @@userguide_layout_path = "userguide_aggregated.html.haml"
      @@userguide_overview_layout_path = "userguide_overview.html.haml"
      
      def initialize(source_path_prefix, target_path_prefix)
        puts "Initializing Userguide"
        @source_path_prefix = source_path_prefix
        @target_path_prefix = target_path_prefix
      end

      def execute(site)
        $LOG.debug "*** Executing userguide extension...." if $LOG.debug?
        userguide_data = Hash.new
        Dir[ "#{site.dir}/#{@source_path_prefix}/*" ].each do |entry|
          if ( File.directory?( entry ) )
            ...
            userguide_data[data_key.to_sym] = data_map 
          end
        end
        # grouping all components' Userguide pages per product id and product version
        site.userguide_pages = Hash.new
        userguide_data.each do |component_id, component_pages|
          # treat .adoc pages
          component_pages.select{|key, component_page| File.extname(key.to_s) == ".adoc"}.each do |key, component_page|
            if site.userguide_pages[component_page.product_id].nil? then
              site.userguide_pages[component_page.product_id] = Hash.new
            end
            userguide_page = get_userguide_page(site, component_page.product_id, component_page.product_version)
            add_component_page(userguide_page, component_page)
            ...
            unless ProductsHelper.is_stable_version(component_page.component_version)
              ...
              unless product_stable_version.nil? 
                userguide_final_page = get_userguide_page(site, component_page.product_id, product_stable_version)
                add_component_page(userguide_final_page, component_page)
              end
            end
          end
        ...
        $LOG.debug "*** Done executing userguide extension...." if $LOG.debug?
      end
      
      def add_component_page(userguide_page, component_page)  
        if userguide_page.component_news[component_page.component_id].nil?
          userguide_page.component_news[component_page.component_id] = Array.new
        end
        userguide_page.component_news[component_page.component_id] << component_page 
      ...
      def get_userguide_page(site, product_id, product_version)
        site.userguide_pages[product_id] = Hash.new if site.userguide_pages[product_id].nil?
        if site.userguide_pages[product_id][product_version].nil? then
          ...
          puts "  building Userguide page for #{product_id} #{product_version}"
          page = create_page(site, @@userguide_layout_path, @target_path_prefix, product_url_path_fragment, product_version)
          ...
          site.userguide_pages[product_id][product_version] = page
        end
        site.userguide_pages[product_id][product_version]
      end
